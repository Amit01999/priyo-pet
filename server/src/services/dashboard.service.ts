...
export async function getDashboardStats(campaign: CampaignDoc): Promise<DashboardStats> {
  ...
  const [totalApplications, statusCounts, paymentStatusCounts, todaysAppointments, dayBreakdown] =
    await Promise.all([
  ...
  for (const row of paymentStatusCounts as { _id: string; count: number }[]) {
    byPaymentStatus[row._id] = row.count;
  }

  return { totalApplications, byStatus, byPaymentStatus, todaysAppointments, today, dayBreakdown };
}
